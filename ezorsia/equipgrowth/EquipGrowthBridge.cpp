...
namespace {
constexpr const char* kVersion = "GROWTH_TIP_GBK_20260818";
...
// Labels in GBK hex to avoid MSVC CP936 mangling. Match set tip naming where possible.
static const StatKey kStatKeys[] = {
        ...
        {L"incMHPMin", "\xD7\xEE\xB4\xF3\xD1\xAA\xC1\xBF", 4}, // 最大血量
        {L"incMMPMin", "\xD7\xEE\xB4\xF3\xC4\xA7\xC1\xBF", 5}, // 最大魔量
...
static constexpr bool kMergeEqualGrowthStats = true;
...
static std::string Utf8ToGbk(const std::string& utf8) {
    ...
    const int wLen = MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, utf8.c_str(), -1, nullptr, 0);
    if (wLen <= 0) {
        return utf8;
    }
    std::wstring wide(static_cast<size_t>(wLen), L'\0');
    if (MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, utf8.c_str(), -1, &wide[0], wLen) <= 0) {
        return utf8;
    }
...
static bool TextHasGbkGrowthTitle(const std::string& text) {
    // GBK 「装备成长」
    return std::strstr(text.c_str(), "\xD7\xB0\xB1\xB8\xB3\xC9\xB3\xA4") != nullptr;
}

/** True when payload is UTF-8 CJK (not Maple GBK). GBK 效=D0A7 looks like UTF-8 Cyrillic Ч. */
static bool LooksLikeUtf8Cjk(const std::string& s) {
    int utf8Cjk = 0;
    for (size_t i = 0; i + 2 < s.size();) {
        const unsigned char c = static_cast<unsigned char>(s[i]);
        const unsigned char c1 = static_cast<unsigned char>(s[i + 1]);
        const unsigned char c2 = static_cast<unsigned char>(s[i + 2]);
        if (c >= 0xE4 && c <= 0xE9 && (c1 & 0xC0) == 0x80 && (c2 & 0xC0) == 0x80) {
            ++utf8Cjk;
            i += 3;
            continue;
        }
...
/**
 * 0x17B text is GBK (same as SetItem skill bonus / writeString zh-CN).
 * Never treat GBK as UTF-8: GBK 「效」D0 A7 becomes Cyrillic Ч then '?'.
 */
static std::string NormalizeTipToGbk(const std::string& inbound) {
    if (inbound.empty()) {
        return inbound;
    }
    if (TextHasLevelSegment(inbound) || TextHasGbkGrowthTitle(inbound)) {
        return inbound;
    }
    if (LooksLikeUtf8Cjk(inbound)) {
        return Utf8ToGbk(inbound);
    }
    return inbound;
}
 ...
 * Format matches set tip: "label : +value"; merge PAD/MAD when equal.
 */
    ...
    for (int node = 1; node < maxItemLevel; ++node) {
        ...
        if (lvl) {
            for (const StatKey& sk : kStatKeys) {
                ...
                if (wcscmp(sk.minKey, L"incPADMin") == 0) {
                    pad = v;
                    continue;
                }
                if (wcscmp(sk.minKey, L"incMADMin") == 0) {
                    mad = v;
                    continue;
                }
                if (wcscmp(sk.minKey, L"incMHPMin") == 0) {
                    mhp = v;
                    continue;
                }
                if (wcscmp(sk.minKey, L"incMMPMin") == 0) {
                    mmp = v;
                    continue;
                }
        ...
        // Emit PAD/MAD (merge when equal) then HP/MP (merge when equal) then others — set order.
        std::vector<std::pair<const char*, int>> ordered;
        if (kMergeEqualGrowthStats && pad != 0 && pad == mad) {
            ordered.emplace_back("\xB9\xA5\xBB\xF7\xC1\xA6/\xC4\xA7\xC1\xA6", pad); // 攻击力/魔力
        } else {
            if (pad != 0) {
                ordered.emplace_back("\xB9\xA5\xBB\xF7\xC1\xA6", pad);
            }
            if (mad != 0) {
                ordered.emplace_back("\xC4\xA7\xC1\xA6", mad);
            }
        }
        if (kMergeEqualGrowthStats && mhp != 0 && mhp == mmp) {
            ordered.emplace_back(
                    "\xD7\xEE\xB4\xF3\xD1\xAA\xC1\xBF/\xD7\xEE\xB4\xF3\xC4\xA7\xC1\xBF", mhp);
        } else {
            if (mhp != 0) {
                ordered.emplace_back("\xD7\xEE\xB4\xF3\xD1\xAA\xC1\xBF", mhp);
            }
            if (mmp != 0) {
                ordered.emplace_back("\xD7\xEE\xB4\xF3\xC4\xA7\xC1\xBF", mmp);
            }
        }
        ordered.insert(ordered.end(), lines.begin(), lines.end());
        if (ordered.empty()) {
            continue;
        }
        anySegment = true;
        ...
        sb += header;
        for (const auto& ln : ordered) {
            char row[80];
            _snprintf_s(row, _TRUNCATE, "%s%s+%d\r\n", ln.first, kGrowthColonSep, ln.second);
            sb += row;
        }
    }
    if (!anySegment) {
...
bool HandleGrowthInbound(void* /*clientSocket*/, CompatInPacket* packet, unsigned short opcode) {
    ...
    c.localOnly = false;
    if (hasData && !text.empty()) {
        // Prefer local WZ GBK 「N级效果」(don't break that path). Else use server bytes:
        // 0x17B is already GBK (SetItem-style); only convert if payload is UTF-8 CJK.
        bool hasSeg = false;
...
int GetCachedItemLevel(int itemId) {
    const auto it = g_cache.find(itemId);
    if (it == g_cache.end()) {
        return 0;
    }
    return it->second.itemLevel > 0 ? it->second.itemLevel : 0;
}
