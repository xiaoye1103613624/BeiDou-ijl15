...
extern "C" __declspec(dllexport) const char* Pendant2_GetStamp() {
    return "PENDANT2_SHOULDER_DUALXY_20260817";
}
...
// FORBIDDEN: 0x00BE27E0 = CUIKeyConfig::s_aptKeyPos (IDA sub_8354E4). Not Equip.
...
static void PatchClassicLayoutDeltas() {
    // Native: when CUIEquip+0x5E8 (extra-pendant/FullBackgrnd) is set, three
    // `and r,0x21` sites add +33px width / pet dock offset. Zero the imm so the
    // window stays classic narrow (175) even if draw-loop max is raised.
...
static void FixZeroDrawSlots() {
    ...
    PatchTableSlot(0x00BE23F0, kShoulderIndex, kClassicShoulderX, kClassicShoulderY);
...
void Pendant2OnClientTick() {
    ...
    PumpExtraPendantFlags();
    // Re-assert gold classic seats (shoulder/rings/pendant2) — races after
    // other UI patches can wipe GetSlotXY/HT and leave red-8 / tip miss.
    // Do NOT write BE27E0 (KeyConfig s_aptKeyPos).
    // Dual-table: flag==0 → BE23F0, flag!=0 → BE2580 (native BP20=(71,233)).
    PatchTableSlot(kClassicGetSlotXyTable, kShoulderIndex, kClassicShoulderX, kClassicShoulderY);
    PatchTableSlot(0x00BE23F0, kShoulderIndex, kClassicShoulderX, kClassicShoulderY);
    PatchTableSlot(kClassicHitTestTable, kShoulderIndex, kClassicShoulderX, kClassicShoulderY);
    // DLL HitTest walk (shoulders redirect) must stay in sync — BE2260 alone is ignored.
    Shoulder_SetExtHitTestSlot(kShoulderIndex, kClassicShoulderX, kClassicShoulderY);
    // Server −20 blind owns 四维 overlay: do NOT NOP 77F98C here (would double-count
    // with STAT_CHANGED). Gold UI coords only; combat skip stays stock jz.
    // Re-assert red cave + rewrite diag once per field (not every tick).
