...
void EnsureHooks() {
    ...
    SidebarLog(line);
    // Fail-safe: world-map tip attach historically never reached sidebar_debug
    // ([WM] lines missing) even though this path always runs. Force it here.
    if (!Client::disableWorldMap) {
        try {
            SidebarLog("EnsureHooks: forcing WorldMapInfo::AttachHooks");
            WorldMapInfo::AttachHooks();
            SidebarLog("EnsureHooks: WorldMapInfo::AttachHooks returned");
        } catch (...) {
            SidebarLog("EnsureHooks: WorldMapInfo::AttachHooks THREW");
        }
    }
