#pragma once
// COutPacket extracted from compat/wvs/packet_legacy.h so this module can
// `using CInPacket = CompatInPacket` without a redefinition clash.
#include "ztl/ztl.h"
...
public:
    ...
    void Init(int nType, int bLoopback, int bTypeHeader1Byte) {
        ...
        if (nType != 0x7FFFFFFF) {
            if (bTypeHeader1Byte) {
                Encode1(static_cast<unsigned char>(nType));
            } else {
                Encode2(static_cast<unsigned short>(nType));
            }
        }
