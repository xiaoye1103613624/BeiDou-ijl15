...
bool ItemNameOf(int itemId, char* out, size_t cap) {
    ...
    auto take = [&](IWzPropertyPtr pNode) -> bool {
        ...
        if (V_VT(&vName) != VT_BSTR || !V_BSTR(&vName) || !V_BSTR(&vName)[0]) {
            return false;
        }
        // String.wz names are narrow in the client UI path; convert from BSTR.
        WideCharToMultiByte(CP_ACP, 0, V_BSTR(&vName), -1, out, static_cast<int>(cap), nullptr,
                            nullptr);
        out[cap - 1] = '\0';
        return out[0] != '\0';
    };
...
CUIColorPrism::CUIColorPrism(int nLeft, int nTop)
    ...
    CreateWnd(m_screenX - m_marginX, m_screenY - m_marginY, CanvasW(), CanvasH(), 10, 1, nullptr,
              1);
