...
static void AppendCombatSyncTotals(std::vector<SetTipLine>& lines) {
    const bool any = SetItemData::g_damR || SetItemData::g_bossDamR || SetItemData::g_normalDamR
            || SetItemData::g_ignorePDR || SetItemData::g_ignoreMDR
            || SetItemData::g_critRate || SetItemData::g_critDam
            || SetItemData::g_padR || SetItemData::g_madR;
    if (!any) {
        return;
    }
    const SetTipStyle atkStyle = SetTipStyle::Orange;
    const SetTipStyle valueStyle = SetTipStyle::White;
    AppendTierStatLine(lines, atkStyle, valueStyle, "\xC9\xCB\xBA\xA6", SetItemData::g_damR, "%");
    AppendTierStatLine(lines, atkStyle, valueStyle,
            "\xB9\xA5\xBB\xF7\xCA\xD7\xC1\xEC\xB9\xD6\xCE\xEF\xCA\xB1\xB5\xC4\xC9\xCB\xBA\xA6",
            SetItemData::g_bossDamR, "%");
    AppendTierStatLine(lines, atkStyle, valueStyle,
            "\xB6\xD4\xC6\xD5\xCD\xA8\xB9\xD6\xCE\xEF\xB5\xC4\xC9\xCB\xBA\xA6",
            SetItemData::g_normalDamR, "%");
    AppendTierStatLine(lines, atkStyle, valueStyle,
            "\xCE\xDE\xCA\xD3\xB9\xD6\xCE\xEF\xB7\xC0\xD3\xF9\xC2\xCA",
            SetItemData::g_ignorePDR, "%");
    AppendTierStatLine(lines, atkStyle, valueStyle,
            "\xCE\xDE\xCA\xD3\xB9\xD6\xCE\xEF\xC4\xA7\xB7\xA8\xB7\xC0\xD3\xF9\xC2\xCA",
            SetItemData::g_ignoreMDR, "%");
    AppendTierStatLine(lines, atkStyle, valueStyle, "PAD%", SetItemData::g_padR, "%");
    AppendTierStatLine(lines, atkStyle, valueStyle, "MAD%", SetItemData::g_madR, "%");
}
...
static void UpdateSetTooltip(CUIToolTip* mainTip, int posX, int posY,
    ...
    try {
        ...
        if (SetItem::IsSetDisabled(setId)) {
            HideSetTooltip();
            g_inTooltipUpdate = false;
            return;
        }
...
void __fastcall CUIToolTip__ClearToolTip_SetItem_hook(
    ...
    if (pThis == g_activeMainTooltip) {
        ...
        // Non-set growth: g_activeMainTooltip may differ; still drop if this tip owns growth.
        EquipGrowth_OnMainTipClearing(pThis);
    }
...
int __fastcall Hook_ShowItemToolTip(
    ...
    if (!a4) {
        ...
        // Double-click use / tip dismiss often takes this path without ClearToolTip
        // matching set-item's active ptr — always drop growth companion.
        EquipGrowth_Hide();
