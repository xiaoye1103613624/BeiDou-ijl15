...
constexpr uintptr_t kVa_TranslateMessageImpl = 0x009E7D77;
constexpr uintptr_t kVa_IWzGr2D = 0x00BF14EC; // read-only for logs; never write Gr2D fields here
...
// CWndMan::TranslateMessageImpl — outer message context for COM logs.
// Modelled as __fastcall(this, edx, MSG*) matching thiscall stack layout.
typedef BOOL(__fastcall* TranslateMessageImpl_t)(void* pThis, void* edx, MSG* pMsg);
TranslateMessageImpl_t g_TranslateMessageImpl =
    reinterpret_cast<TranslateMessageImpl_t>(kVa_TranslateMessageImpl);

BOOL __fastcall Hook_TranslateMessageImpl(void* pThis, void* edx, MSG* pMsg) {
    if (pMsg) {
        __try {
            CrashDiag_SetMsgContext(GetCurrentThreadId(), pMsg->message, pMsg->wParam, pMsg->lParam);
        } __except (EXCEPTION_EXECUTE_HANDLER) {
        }
    }
    const BOOL r = g_TranslateMessageImpl(pThis, edx, pMsg);
...
void CrashDiag_AttachHooks() {
    ...
    // TranslateMessageImpl 钩已验证会踩栈：崩溃 EIP=0x009E8065（钩点 0x009E7D77 附近）。
    // 先保持关闭；消息上下文仅依赖其它路径手动 SetMsgContext（若有）。
    // Memory::SetHook(true, reinterpret_cast<void**>(&g_TranslateMessageImpl),
    //                 reinterpret_cast<void*>(&Hook_TranslateMessageImpl));
}
...
void CrashDiag_DebugTriggerEPointer() {
    typedef void(__cdecl* ComRaiseError_t)(HRESULT hr, IErrorInfo* pInfo);
    auto fn = reinterpret_cast<ComRaiseError_t>(kVa_ComRaiseError);
    try {
        fn(static_cast<HRESULT>(0x80004003), nullptr);
    } catch (...) {
        // original _com_raise_error throws; allow catch for COM-log verification
    }
}
