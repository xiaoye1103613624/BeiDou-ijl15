...
//
// Cost of single-sourcing Band B/C from FullBackgrnd: the default 4x6 window is
// geometry-identical to stock but its cell paper tone comes from FullBackgrnd
// rather than backgrnd. Band A (title + tab plate) uses Item/backgrnd for the
// title and a glyph-free gutter fill for the tab strip so CCtrlTab is the only
// source of the five labels -- tiling FullBackgrnd's top band reprinted ghost
// 「特殊」/「消耗」 glyphs on top of the live tabs.
//
// ---------------------------------------------------------------------------
...
// 20 rows lets common 120-cell shapes land (12x10, 10x12, 8x15, 6x20) while
 // the per-frame screen clamp still shrinks anything that would go off-screen.
constexpr int kMinRows = 3,  kMaxRows = 20;
...
// Clamp to the legal rectangle AND the 120-cell imm8 ceiling. Each step shrinks
// the axis that leaves the larger remaining product so a (16,20) request lands
// on 15x8=120 rather than collapsing far under the cap.
void ClampGrid(int& cols, int& rows) {
    cols = Clamp(cols, kMinCols, kMaxCols);
    rows = Clamp(rows, kMinRows, kMaxRows);
    while (cols * rows > kMaxCells) {
        const int withoutCol = (cols > kMinCols) ? (cols - 1) * rows : -1;
        const int withoutRow = (rows > kMinRows) ? cols * (rows - 1) : -1;
        if (withoutCol < 0 && withoutRow < 0) break;
        if (withoutCol >= withoutRow) --cols;
        else --rows;
    }
}
...
// ===========================================================================
// Background compositor. See the header for why Band B/C come from
// FullBackgrnd. Band A (title + tab plate) is special: FullBackgrnd bakes the
 // *full-mode* multi-group tab glyphs into its top 50px, and tiling that strip
 // was reprinting ghost 「特殊」/「消耗」 labels on top of the live CCtrlTab.
 // Narrow Item/backgrnd supplies the title chrome; the tab plate is filled from
 // a glyph-free gutter column so only CCtrlTab draws the five labels.
// ===========================================================================
constexpr int kSrcW = 603;        // FullBackgrnd width
constexpr int kNarrowW = 175;     // Item/backgrnd width
constexpr int kTilePeriod = 32;   // measured: each tiled band alternates two
                                  // 16px flat runs, so the true period is 32

constexpr int kA_H = 50, kA_Right = 12;
constexpr int kA_TitleH = 22;         // rows 0..21: title bar only (no tab glyphs)
constexpr int kA_TitleLeft = 70;      // covers 「现有道具」; stops before button art
constexpr int kA_TitleTileX = 88;     // blank title paper between text and buttons
constexpr int kA_TabFillX = 4;        // left gutter of the tab plate (no glyphs)
...
IWzCanvas* NarrowBackgrnd() {
    static bool bTried = false;
    static IWzCanvasPtr pNarrow;
    if (!bTried) {
        bTried = true;
        try {
            pNarrow = get_unknown(get_rm()->GetObjectA(
                const_cast<wchar_t*>(L"UI/UIWindow.img/Item/backgrnd")));
        } catch (...) {
        }
        if (!pNarrow) {
            LOG_ONCE("invresize: UI/UIWindow.img/Item/backgrnd is missing; Band A falls back "
                     "to FullBackgrnd title (tab ghosts may return)");
        }
    }
    return pNarrow;
}
...
IWzCanvasPtr BuildBackground(int cols, int rows) {
    ...
    IWzCanvas* pN = NarrowBackgrnd();
    // Title/tab plate prefer the narrow stock chrome; FullBackgrnd is only the
    // fallback when Item/backgrnd is absent (and then tab ghosts can return).
    IWzCanvas* pA = pN ? pN : pF;
    ...
    try {
        ...
        const int midEnd = W - kA_Right;

        // Band A -- title bar (glyph-bearing) + blank tab plate (CCtrlTab draws
        // the five labels; never tile FullBackgrnd's baked multi-group tabs).
        Blit(pDst, pA, 0, 0, kA_TitleLeft, kA_TitleH, 0, 0);
        if (kA_TitleLeft < midEnd) {
            TileH(pDst, pA, kA_TitleTileX, 0, kA_TitleH, kA_TitleLeft, midEnd, 0);
        }
        Blit(pDst, pF, kSrcW - kA_Right, 0, kA_Right, kA_TitleH, W - kA_Right, 0);

        // Tab plate: fill the whole strip from a gutter column that has the red
        // rule / gray plate but no Chinese tab glyphs.
        TileH(pDst, pA, kA_TabFillX, kA_TitleH, kA_H - kA_TitleH, 0, midEnd, kA_TitleH);
        Blit(pDst, pA, 0, kA_TitleH, kB_RailW, kA_H - kA_TitleH, 0, kA_TitleH);
        Blit(pDst, pF, kSrcW - kA_Right, kA_TitleH, kA_Right, kA_H - kA_TitleH,
             W - kA_Right, kA_TitleH);
...
void InstallBackground(void* pThis, int cols, int rows) {
    // Cached on (cols, rows, recipe): the window is destroyed and rebuilt on
    // every snap step of a drag, and recompositing hundreds of blits per step
...
void ApplySize(int cols, int rows) {
    ...
    ClampGrid(cols, rows);
    if (cols == g_nCols && rows == g_nRows) {
        // Same grid: still refresh Band A so a DLL upgrade (or a prior ghosted
        // FullBackgrnd tile) does not keep overlapping tab glyphs until the
        // player happens to drag the edge.
        const int W = SehReadInt(pThis, kOff_Width);
        const int H = SehReadInt(pThis, kOff_Height);
        if (W == WndW(cols) && H == WndH(rows)) {
            InstallBackground(pThis, cols, rows);
            Invalidate(pThis);
            return;
        }
    }
...
void LoadSizeOnce() {
    ...
    if (!pCfg) return;
    g_bLoaded = true;
    int c = pCfg->GetOpt_Int(CConfig::CHARACTER_OPT, "invCols", kMinCols, kMinCols, kMaxCols);
    int r = pCfg->GetOpt_Int(CConfig::CHARACTER_OPT, "invRows", 6,        kMinRows, kMaxRows);
    ClampGrid(c, r);
    if (c != g_nCols || r != g_nRows) {
...
// The button (id 0x7D5) becomes a quick jump between stock 4x6 and the largest
// on-screen grid under the 120-cell ceiling (nearest thing to the old "make
// the inventory wide" meaning, without exceeding the imm8 patch budget).
void __fastcall ToggleFull_hook(void* /*pThis*/, void* /*edx*/) {
    if (g_nCols * g_nRows > 48) {
        g_pendCols = kMinCols;
        g_pendRows = 6;
        return;
    }
...
bool InvResize_HandleMouseMessage(UINT& msg, WPARAM, LPARAM, LRESULT*) {
    ...
    if (msg == WM_MOUSEMOVE && g_bDragging) {
        ...
        // Keep the whole window on screen, then re-clamp so a screen shrink
        // cannot leave a stale over-ceiling pair behind.
        const int sw = get_screen_width(), sh = get_screen_height();
        while (cols > kMinCols && absL + WndW(cols) > sw - 8) --cols;
        while (rows > kMinRows && absT + WndH(rows) > sh - 8) --rows;
        ClampGrid(cols, rows);
